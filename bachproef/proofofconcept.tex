        ...
        withCredentials([string(credentialsId: 'nvd-api-key', variable: 'NVD_API_KEY')]) {
            catchError(buildResult: 'UNSTABLE', stageResult: 'FAILURE') {
                sh '''
                    ...
                    curl -L https://github.com/dependency-check/.../v12.1.0/dependency-check-12.1.0-release.zip -o dependency-check.zip
          ...
          stage('Install TruffleHog'){
            steps {
              sh '''
                ...
                curl -sSfL https://raw.githubusercontent.com/trufflesecurity/.../install.sh | sh -s -- -b .
                '''
              }
            }
...
Figuren~\ref{fig:jenkins-run-overview-pt1} en~\ref{fig:jenkins-run-overview-pt2} tonen het stage-overzicht van de pipeline voor deze run. De klassieke CI-stappen (\texttt{Tool Install}, \texttt{Checkout}, \texttt{Maven Build} en \texttt{Build Container Image}) slagen zonder fouten, waarna de parallelle securityflows, runtime-stappen en archivering worden uitgevoerd.

De totale doorlooptijd van deze run bedroeg 7 minuten en 28 seconden.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../graphics/jenkins-run-overview-pt1.png}
    \caption{Overzicht van de Jenkins-pipeline voor de referentierun van Spring Petclinic, deel 1.}
    \label{fig:jenkins-run-overview-pt1}
\end{figure}
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../graphics/jenkins-run-overview-pt2.png}
    \caption{Overzicht van de Jenkins-pipeline voor de referentierun van Spring Petclinic, deel 2.}
    \label{fig:jenkins-run-overview-pt2}
\end{figure}

\subsubsection{Securitybevindingen per tool}
De referentierun laat zien dat de verschillende securityflows effectief problemen detecteren in de applicatie en containerimage. In de stage \texttt{Parallel Security Checks} werden drie stappen als fout gemarkeerd: \texttt{Dependency Check Scan}, \\\texttt{SBOM Validatie (BSI TR-03183-2)} en \texttt{Scan Container Image}. Deze failures duiden op gevonden kwetsbaarheden of validatieproblemen, terwijl de andere securityflows (SonarQube, SBOM-generatie, TruffleHog) succesvol afronden.
...
De referentierun met de kwetsbare Python-applicatie, afgebeeld in Figuur~\ref{fig:jenkins-run-python-vuln} en de referentierun zonder kwetsbaarheden, afgebeeld in Figuur~\ref{fig:jenkins-run-python}, tonen dat de pipeline effectief onderscheid maakt tussen applicaties met en zonder kwetsbaarheden. In beide runs falen de stappen \texttt{SBOM Validatie} en \texttt{Scan Container Image}, doordat de gegenereerde SBOM niet volledig aan de vereisten voldoet en het containerimage kwetsbaarheden bevat. Het belangrijkste verschil tussen beide scenario's zit in \texttt{Dependency Check Scan}: in de kwetsbare pipeline wordt deze stage als fout gemarkeerd, terwijl de niet-kwetsbare variant hier geen relevante CVE's rapporteert en dus slaagt.

De DAST-stap met OWASP ZAP bleef in deze test in de vulnerable Python applicatie buiten scope, omdat de Python-applicatie in deze opzet niet als actieve webserver werd opgestart. Daardoor ligt het onderscheid tussen beide runs vooral in de resultaten van de dependency-analyse en niet in de runtime-scanning.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../graphics/jenkins-run-python-vuln.png}
    \caption{Referentierun kwetsbare Python-applicatie.}
    \label{fig:jenkins-run-python-vuln}
\end{figure}
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../graphics/jenkins-run-python.png}
    \caption{Referentierun niet-kwetsbare Python-applicatie.}
    \label{fig:jenkins-run-python}
\end{figure}
\FloatBarrier
...
De referentierun met de kwetsbare .NET-applicatie, afgebeeld in Figuur~\ref{fig:jenkins-run-dotnet-vuln} en de referentierun zonder kwetsbaarheden, afgebeeld in Figuur~\ref{fig:jenkins-run-dotnet}, tonen hetzelfde patroon als bij de Python-applicaties. De pipeline maakt effectief onderscheid tussen een kwetsbare en een niet-kwetsbare versie. In beide runs falen de stappen \texttt{SBOM Validatie} en \texttt{Scan Container Image} om exact dezelfde redenen als vermeld in Subsectie~\ref{ch:python-apps}. En ook hier is het belangrijkste verschil in stage \texttt{Dependency Check Scan}: in de kwetsbare pipeline is deze stap als fout gemarkeerd en bij de niet kwetsbare zijn er geen relevante CVE's gevonden. Ook dit is dus als beschreven in Subsectie~\ref{ch:python-apps}.

Ook hier bleef de DAST-stap met OWASP ZAP buiten scope bij de vulnerable .NET-applicatie.
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../graphics/jenkins-run-dotnet-vuln.png}
    \caption{Referentierun kwetsbare .NET-applicatie.}
    \label{fig:jenkins-run-dotnet-vuln}
\end{figure}
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{../graphics/jenkins-run-dotnet.png}
    \caption{Referentierun niet-kwetsbare .NET-applicatie.}
    \label{fig:jenkins-run-dotnet}
\end{figure}
\FloatBarrier
