...
De Cyber Resilience Act (CRA), die op 10 december 2024 in werking trad, is een Europese wetgeving die de cybersecurity van digitale producten verplicht stelt vóór ze op de markt komen \autocite{EU_CRA}. Het doel hiervan is het verbeteren van de digitale weerbaarheid van producten, met andere woorden: het vermogen van deze producten om veilig en betrouwbaar te blijven functioneren ondanks cyberdreigingen en het verkleinen van de risico's door kwetsbaarheden in software en hardware op tijd te detecteren en mitigeren. 

\subsection{Specifieke vereisten en tijdlijn}
De CRA legt strikte eisen op doorheen de volledige levenscyclus: security-by-design (risicoanalyse en secure development life cycle), Software Bill of Materials (SBOM) voor transparantie van componenten, vulnerability management (CVE-scans en mitigatie), incidentrapportage binnen 24 uur bij ernstige kwetsbaarheden en verplichte beveiligingsupdates gedurende minstens 5 jaar \autocite{EU_CRA}. De implementatietijdslijn verloopt in fasen: meldplicht voor ernstige kwetsbaarheden start op 11 september 2026, volledige toepassing van alle eisen inclusief CE-certificering vanaf 11 december 2027 \autocite{RDI2024CRA}.
...
\begin{figure}[h]
  ...
  \end{tikzpicture}
  \caption[Implementatietijdslijn CRA]{Implementatietijdslijn van de Cyber Resilience Act, op basis van de officiële CRA-tijdslijn \autocite{RDI2024CRA,EuropeanCommission2024CRAnews}.}
...
SBOM's worden gegenereerd door tools die software dependencies analyseren tijdens de build-fase van CI/CD pipelines. \textcite{Mirakhorli2024} evalueerden 84 SBOM tools en identificeerden de tools met de beste prestaties op basis van taal/ecosysteem ondersteuning.
...
CVE-Records worden verrijkt door de National Vulnerability Database (NVD) met gestructureerde metadata voor geautomatiseerde matching. De NVD bevat naast de CVE-details ook \textbf{CVSS-scores} (score van 0 tot 10, hoe hoger hoe ernstiger) en \textbf{CPE-matchcriteria} (gestandaardiseerde identifiers voor getroffen software of\\hardware) en referenties naar getroffen producten \autocite{NVD_API}.
...
De matching-flow is geïllustreerd in~\ref{fig:cpe-matching-flow}:
...
\begin{figure}[h]
  ...
  \end{tikzpicture}
  \caption{Matching-flow SBOM naar CVE-detectie via NVD CPE-matching \autocite{NVD_API}}
...
Vervolgens, geïllustreerd is de verschillende scores met bijhorend level in~\ref{tab:cvss-v4-scores}
\begin{table}[H]
  \centering
...
DAST-tools voeren dynamische security testing uit op runtime-applicaties, waarbij ze zich gedragen als een externe aanvaller die probeert kwetsbaarheden te exploiteren. Verschillende studies evalueerden DAST-tools op basis van detectiecapaciteiten, false positive ratio en CI/CD-integratie \autocite{AIMultipleDAST2025,AppSecSanta2025}.
...
Vervolgens een korte samenvatting van alle besproken tools~\ref{tab:tool-comparison}:
\begin{table}[h]
  \centering
...
\begin{enumerate}
  \item \textbf{SBOM-generatie}: Trusted Profile Analyzer produceert CycloneDX SBOM's
  ...
  \item \textbf{Audit-ready rapportage}: SBOM's en scan-resultaten worden gearchiveerd voor CRA-documentatie (zie Figuur~\ref{fig:redhat-pipeline})
\end{enumerate}
...
Deze aanpak voldoet aan de vereisten van CRA door machine-readable SBOM's te maken alsook vulnerability-data te koppelen \autocite{RedHatPipeline}.
...
Deze pipeline produceert CycloneDX/SPDX SBOM's, scant voor CVEs en genereert audit-ready rapporten. Dit voldoet aan de BSI TR-03183-2 (de eisen voor de Duitse Overheid ivm. Cyber Resilience) en is direct bruikbaar voor Jenkins \autocite{CRAEvidencePipeline}. 
